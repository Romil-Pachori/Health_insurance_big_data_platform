...
from src.transformations.staging import run_synthesis
from src.utils.config import load_config, run_date_or_default
from src.utils.spark import build_spark


def main() -> None:
    parser = argparse.ArgumentParser(description="Run deterministic event synthesis from staging aggregates.")
    ...
    spark = build_spark("health-insurance-synthesis")
    try:
        run_synthesis(spark, config, resolved_run_date)
    finally:
        spark.stop()
